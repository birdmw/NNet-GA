...
class Synapse:
    ...
    def run(self):
        if self.n1.isOutput == 0 and self.n2.isInput == 0: #If not an input and not an output
            sinFxn = max(min(self.propertyList[0] * sin(self.propertyList[1] * self.n1.outbox + self.propertyList[2]),1000),-1000)
            diffFxn = max(min(self.propertyList[3] * (self.n1.prevOutbox - self.n1.outbox),1000),-1000)
